...
web_out_path = config.get('mount_path', 'web_out_path')
metadata_outpath = config.get('mount_path', 'metadata_outpath')
silicone_elast_path = config.get('mount_path', 'silicone_elast_path')
...
def Control_table_website_check(sql_conn):
  try:
    ...
    #delete_query = "delete from momentive.website_control_table where source_type = 'Website'"
    ...
    if sql_conn is not None: 
      select_query = "select * from momentive.website_control_table where source_type = 'Website'"
      web_df = pd.read_sql(select_query, sql_conn)
      logger.info('Successfully extracted the data of momentive.website_control_table for website from sql server')
    else:
  ...
  try:
    ...
    if web_df.empty:
      for i in metadata_list.index:
        insert_query = '''insert into momentive.website_control_table (is_updated_file,  source_type, file_name, file_type, created, updated, file_size, website_path) values ( 0 , 'Website', '{}' , '{}', '{}', '{}', '{}', '{}') '''.format(metadata_list['File_name'][i][:-4], metadata_list['File_name'][i][-3:], metadata_list['Date'][i], metadata_list['Date'][i], metadata_list['File_size'][i], metadata_list['website_path'][i])
    ...
    #CRUD operation in the control audit table for incremental load
      ...
      for i in metadata_list.index:
        ...
        #Update operation
          ...
          update_query = '''update momentive.website_control_table set updated = '{}', file_size = '{}', is_updated_file = 1 where file_name = '{}' and source_type = '{}' '''.format(metadata_list['Date'][i], metadata_list['File_size'][i], metadata_list['File_name'][i][:-4], 'Website')
      ...
      if bool(file_exist_audit_check):
        ...
        for file_name in file_difference:
          delete_query = '''delete from momentive.website_control_table where source_type = '{}' and file_name ='{}' '''.format('Website',file_name)
          cursor.execute(delete_query)
          sql_conn.commit()
