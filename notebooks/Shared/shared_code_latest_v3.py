    ...
    if os.path.exists(valid_path + 'relevant_data_files/'):
      ...
      for file in files:
        ...
        for i in range(key_value_df_master_data.shape[0]):
          unstructure_processed_data(unstructure_processed_data_query,category,key_value_df_master_data['Product_category'][i],key_value_df_master_data['Product'][i],key_value_df_master_data['values'][i].replace("'", "''"),key_value_df_master_data['is_relevant'][i],sql_conn,cursor)
          
  except Exception as e:
    print(e)
    
def data_validation_to_relevant_non_relevant_split(data_delta, valid_path, primary_column, comp,sql_conn,cursor):
  try:
...
def reading_excel_data_from_source(valid_path, files, component_data, primary_column, comp,sql_conn,cursor):
  ...
  try:
    ...
    data_validation_to_relevant_non_relevant_split(data_delta, valid_path, primary_column, comp,sql_conn,cursor)
  except Exception as e:
...
def excel_extract2_key_value_pair(valid_path, sql_conn,cursor,category):
  try:
              ...
              for files in valid_files:
                ...
                if file_name.strip()==sheet.strip():
                  for primary in primary_col:
                    reading_excel_data_from_source(valid_path, files, component_data, primary, comp,sql_conn,cursor)
            else:
          ...
          for pattern_cat_match in pattern_category:
              ...
              if  pattern_match_flag == 's':      
                  ...
                  #******************************************************************************************  
                      ...
                      if file.strip().endswith('.pdf'):# and not file.endswith('.xlsm') and not file.endswith('.csv'):
                        file_is_valid = file_is_valid_query.format(1,1,'null',file.replace("dbfs:","/dbfs"))
                        update_operation(file_is_valid,sql_conn,cursor)
                        file = external_staging_path[index] + file.rsplit('/',1)[1].rsplit('.',1)[0] + \
                        external_satging_file_format[index]
                        relavent_image_extract(file,file_loc,content,product_inscope_df,pattern_cat_match.strip(),file_is_valid_query, 
...
#           dbutils.fs.rm((staging_path +'temp/csv/').replace("/dbfs",""),True)
          ...
          #update_operation
          #********************************************************************************************************************
          file_processing_info_query = file_processing_info + " values ('{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', {}, {},'{}')".format(source_type, file_name, 'Excel','.csv', staging_path.replace('//','/'), 'null', 0,0,'null','null','null')
          update_operation(file_processing_info_query,sql_conn,cursor)
...
#           dbutils.fs.rm((staging_path +'temp/csv/').replace("/dbfs",""),True)
          ...
          #if os.path.exists(staging_path +'temp/temp_all_text/'):
          path_exists(staging_path +'temp/temp_all_text/')
                    
          for i in range(len(temp_path)):
              #print(temp_path[i].replace("/dbfs","dbfs:"), (excel_files + file_name+'/').replace("/dbfs","dbfs:"))
