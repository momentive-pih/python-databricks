...
fh = logging.FileHandler("shared_main_code3.log", 'w')
fh.setLevel(logging.DEBUG)
ch = logging.FileHandler("shared_main_code_error3.log", 'w')
  ...
  try:
        ...
        for tab in tables:
            ...
            if data.shape[1] > 2:
                ...
                name_col = data[data.text1.str.startswith('Name:')]['text1'].values.tolist()                
                for name_det in name_col:                  
                  name_co11.append(name_det.replace('Name:', '').strip())
            else:
                n = data.shape[1] + 1
                data.columns = ['text'+ str(i) for i in range(1, n)]   
                name_col = data[data.text1.str.startswith('Name:')]['text1'].values.tolist()                
                        ...
                        try:
                            if not str(product_inscope_df['Text'][prod_index].strip()).isspace():
                              prod_rgx = re.search(r'(([^a-zA-Z]|^){}[^a-zA-Z])'.format(re.escape(product_inscope_df['Text']
                                         [prod_index].strip())),name_mul[1],re.I)                                   
                              if(prod_rgx):
                              ...
                              logger.error("Error in relavent_file function inner iteration while processing\
                    ...
                    if index_col != '':
                      for tab_index in df.index:
                        ...
                        unstructure_processed_data(unstructure_processed_data_query,category,product_inscope_df['Type']
                                                 [index_col].strip(),name_mul[1],data_extract,1,sql_conn,cursor)
                    else:
                      for tab_index in df.index:
                        data_extract={}
                        for   inc_col in  inci_columns: 
                        ...
                        'null',name_mul[1],data_extract,1,sql_conn,cursor)
            else:
                print('name_co11',name_co11)
                print('data.shape[0]',data.shape[0])
                name_mul = name_co11 *  (data.shape[0]-1) 
                name_mul.insert(0,'Product Name')
                print('name_mul',len(name_mul))
                      ...
                      if not pd.isnull(product_inscope_df['Type'][prod_index]):
                        try:
                            if not str(product_inscope_df['Text'][prod_index].strip()).isspace():
                              ...
                              logger.error("Error in relavent_file function inner iteration while processing \
                                           {}".format(table_file),exc_info=True)
                                  #dbutils.fs.mkdirs((master_path +'output/').replace("/dbfs","dbfs:")) 
                                 #df.to_csv(master_path + 'output/'+ file_name + '.csv', index= False, header=None, encoding= 'utf-8')
...
#called by: pattern_match_validation
#**************************************************************************************************************************************   
def relavent_text_extract(file,staging_file,file_loc,content,product_inscope_df,category,file_is_valid_query,file_unique_list,sql_conn,cursor,unstruct_category_key_df,raw_df,external_processed_files_df_rep,unstructure_processed_data_query,sil_elast_product_list= None):
  try: 
      ...
      staging_raw_file_path = staging_file
#       try:
      ...
      try:
        ...
        if file in extracted_file_list:
          ...
          for pattern_cat_match in pattern_category:
              ...
              #****************************************************
              print('pattern_match_flag',pattern_match_flag)
                  ...
                  if all(int(match.lower().strip()) == 0 for match in pattern_result_list):                         
                      ...
                      if external_satging_file_format[index].strip()  in ['.pdf','.docx','.doc','.pptx']:
                        ...
                        relavent_text_extract(file,staging_file,file_loc,content,product_inscope_df,pattern_cat_match.strip(),
                          ...
                          raw_df,external_processed_files_df_rep,
            ...
            #    pdf_file_list = ['/dbfs/mnt/momentive-sources-pih/sharepoint-pih/customer-communications-pih/mpm-2019-pih/staging/pdf/raw/Polydimethylsiloxane - Review 6974.pdf']
                ...
                if bool(pdf_file_list):
                    ...
                    for i in pdf_file_list:
                      pdf_check1='Silsoft 253 CIDP (P)'
                      if pdf_check1 in i:
                        pdf_file_list1.append(i) 
#                     raw_files = raw_files + pdf_file_list1
#                    raw_format = raw_format + ['.pdf']*len(pdf_file_list1)
                    native_path, scanned_path = sharepoint_native_scanned_pdf_split(staging_path,pdf_file_list1)
                    if native_path != None:
                     #***************************************************************************************************
...
# MAGIC %sh
# MAGIC cat /databricks/driver/shared_main_code_error3.log
...
print(os.path.exists('/dbfs/mnt/momentive-sources-pih/sharepoint-pih/cognizant-pih/ag-reg-status-pih/analytics/processed/excel/Master Registration List_Cognizant/EU/'))
