...
def key_data_extract_external_source(valid_path,sql_conn,cursor,category,unstructure_processed_data_query):
    ...
    if os.path.exists(valid_path + 'relevant_data_files/'):
      ...
      for file in files:
        ...
        for i in range(key_value_df_master_data.shape[0]):
          unstructure_processed_data(unstructure_processed_data_query,category,key_value_df_master_data['Product_category']
                                     [i],key_value_df_master_data['Product'][i],key_value_df_master_data['values'][i].replace("'", 
...
#     bdt_list = list(product_inscope.bdt)
    ...
    regex2 = re.compile(r'(\w+-\d+)', re.I)  #Y-Number formatting
...
def excel_full_delta_load(valid_path, relevant_data,sql_conn,cursor):
...
def reading_excel_data_from_source(valid_path, files, component_data, primary_column, comp,sql_conn,cursor,product_inscope_df):
  ...
  try:
    ...
    data_delta, valid_path = excel_full_delta_load(valid_path, relevant_data,sql_conn,cursor)
...
def excel_extract2_key_value_pair(valid_path, sql_conn,cursor,category,product_inscope_df):
  try:
      ...
      pass
          ...
          for sheet in component_sheet:
            if sheet:
              ...
              logger.info("sheets not available in the {}".format(valid_path),exc_info=True)     
  except Exception as e:
...
def text_Key_extract(file,filter_df,content):
  try: 
    ...
    for  index_df in filter_df.index:
        ...
        if  start_string_index is not None:
            ...
            if filter_df['end_key'][index_df] != 'date_type':
                ...
                rgx ="|".join(["\d{1,2}\s*\/\d{1,2}\s*\/\d{4}","[a-zA-Z]*\s*\d{1,2}\s*,\s*\d{4}",
                               "\d{1,2}\s[a-zA-Z-]*\s\d{4}","\d{1,2}\s*\-\s*[a-zA-Z]*\s*\-\s*\d{4}"])
    ...
    #date extract
    #****************
    rgx_pattern_date =["\d{1,2}\s*\/\d{1,2}\s*\/\d{4}","[a-zA-Z]*\s*\d{1,2}\s*,\s*\d{4}","\d{1,2}\s[a-zA-Z-]*\s\d{4}",
        ...
        if not pd.isnull(product_inscope_df['Type'][prod_index]):
           try:
              ...
              if(prod_rgx):
                  ...
                  if prod_txt.lower().strip() not in product_list_lower:
        ...
        if file in extracted_file_list:
          ...
          for pattern_cat_match in pattern_category:
              ...
              if  pattern_match_flag == 's':      
                  ...
                  #*****************************************************************************************
                  if all(int(match.lower().strip()) == 0 for match in 
                         (pattern_key_df[pattern_key_df['pattern_category'].str.contains("^\s*{}\s*$"
                         .format(pattern_cat_match),case=False)])                                                       
                         ['result_type'].values.tolist()):
                      ...
                      relavent_text_extract(file,file_loc,content,product_inscope_df,pattern_cat_match.strip(),
                      file_is_valid_query,file_unique_list,sql_conn,cursor,unstruct_category_key_df,raw_df,
                      unstructure_processed_data_query,sil_elast_product_list)       
                      file_valid_flag ='s'
                  ...
                  #******************************************************************************************  
                      ...
                      file1 = external_staging_path[index] + file.rsplit('/',1)[1].rsplit('.',1)[0] + external_satging_file_format[index]
                      print(file)
                  ...
                  #******************************************************************************************  
                      ...
                      unstructure_processed_data_query)
                      file_valid_flag ='s'   
...
def excel2txt(staging_path, abs_path):
  try:
    ...
    file = staging_path +'temp/temp_all_text/' + abs_path.rsplit('/',1)[1].replace('.csv','.txt')
  ...
  try:
      ...
      for abs_path in csv_list:
        try:
          ...
          if os.path.exists(excel_files + file_name +'/'):
          ...
          #************************************************************** 
            if '.csv ' in external_source_file_formats:
                ...
                if bool(csv_list):
                  ...
                  csv_text_extract(staging_path,csv_list,source_type,all_files,file_processing_info,update_file_processing_info,
                  file_processing_blob_all_txt_list,sql_conn,cursor)
          #*******************************************************************************
