...
#       pos_filter=[word for word,tag in tag_item if tag in ["NNP","NN","CD"]]
    ...
    check_ontology_column=["ontology_key","ontology_value","key_type"]
    row_product=[]
    argument_str=[]
    starting_indx=-1
    ...
    def creating_argument_value(input_value,starting_indx,history_flag):
      try:
        temp_row=[]
        for category_type,item,subct in input_value:
          try:
            item=str(item.strip())
            item=item.replace("'","")
            ...
            if (item !='' and item.lower() !="null"):
              temp_str=category_type+"---"+item+"---"+subct+"---"+str(starting_indx)+"---"+history_flag
    ...
    def update_ontology(to_be_checked):
      for ontology_key,ontology_value,key_type in to_be_checked:
        try:
    ...
    if (len(inscope_sfdc_info_df)>0 and os.path.exists(processing_file_name)):
      ...
      if len(ontology_product_df)==len(inc_ontology_product_df):
        ...
        argument_str=creating_argument_value(to_be_checked,starting_indx,history_flag)
        row_product=row_product+argument_str
        argument_str=creating_argument_value(to_be_checked,starting_indx,"false")
        row_product=row_product+argument_str
        update_ontology(to_be_checked)
